...
public class DateUtil {
    ...
    /**
     * 获取指定时间 指定格式    间距多少分钟后的时间  字符串
     *
     * @param datestr
     * @param diff
     * @return
     */
    public static String getDiffMinuteDateStr02(String datestr, Integer diff) {
        Date d = getDateFromStr("yyyy-MM-dd HH:mm:ss", datestr);
        Calendar now = Calendar.getInstance();
        now.setTime(d);
        now.set(Calendar.MINUTE, now.get(Calendar.MINUTE) + diff);
        Date rd = now.getTime();
        return y_m_d_h_m_sFormat.format(rd);
    }

    /**
     * 获取指定时间 指定格式 间距多少秒后的时间 字符串
     * @param datestr
     * @param diff
     * @return
     */
    public static String getDiffSecondDateStr(String datestr, Integer diff) {
        System.out.println("[getDiffSecondDateStr] diff :" + diff);
        Date d = getDateFromStr("yyyy-MM-dd HH:mm:ss", datestr);
        Calendar now = Calendar.getInstance();
        now.setTime(d);
        now.set(Calendar.SECOND,now.get(Calendar.SECOND) + diff);
        Date rd = now.getTime();
        System.out.println("[getDiffSecondDateStr] finish:" + y_m_d_h_m_sFormat.format(rd));
        return y_m_d_h_m_sFormat.format(rd);
    }
    /**
     * 当前时间天数+-
